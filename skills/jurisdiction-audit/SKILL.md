...
4. **服务范围只有中国大陆**。识别到港澳台或任何境外法域，输出「不在本团队服务范围」
   并终止，不出任何实体合规结论。`jurisdiction-us` 包仅用于识别越界，不用于给结论。
5. **引用任何条号前，先过 `statutes/temporal.yaml` 的版本区间校验**（J2.5 闸门二）。
   合同签署日不在本库收录版本的有效区间内 ⇒ 禁止引用本库条文。
   法不溯及既往：一份 2019 年的合同适用的是当时的法，不是今天的现行法。
6. **法域未确定 → 只报「法域待确认」**（`verdict: jurisdiction_undetermined`），不出任何合规结论，不给「初步」「暂按」「倾向于」版本。
7. **`jurisdiction_pack_version` 与法域线索不一致 → 阻断**（`INV-008` / `rules.md#R-021`），不是警告、不是标注后继续。
8. **不确定的法条写「需人工确认」**，置信度原样转述。**禁止编造条文编号、司法解释文号与判例名称**（`rules.md#R-061`）。
9. **每条结论齐备四元组**：条款编号 + 证据位置（页码）+ 结论等级 + 对应动作。缺任一项该条不合格。
10. **没检查到就显式留白**，`blank` / `unknown` 并写原因，禁止按通过计（`INV-012`）。
...
## J2.5 服务范围闸门与法律版本判定（**J3 之前必须完成**）

这一步在 J2 定出法域之后、J3 加载规则包之前。两道闸门，任一不过都不进 J3。

### 闸门一：服务范围

本团队**只服务中国大陆**（见 `jurisdiction-cn/pack.yaml#service_scope`）。

| J2 判定的法域 | 处理 |
|---|---|
| 中国大陆 | 通过，进闸门二 |
| 港澳台 | **终止**。三地法律体系不同，不是「中国法的地方变体」 |
| 任何境外法域 | **终止**。输出「该合同法域为 X，不在本团队服务范围」 |
| `undetermined` | 按 J2 原有出口处理，不进 J3 |

> `jurisdiction-us` 包只用来**认出**越界（`usage: detection_only`），
> 包内规则、标尺、州法说明**一律不得进入报告结论**。
> ⚠️ 宁可明确拒绝，也不要给一份没有法域依据、却看起来像模像样的报告——
> 后者是这条边界上最坏的失败模式。

### 闸门二：法律版本（法不溯及既往）

**先定合同签署日，再定用哪一版法。** 一份 2019 年签的合同，适用的是当时有效的
法律，不是今天的现行法。法律依据是《民法典时间效力若干规定》第一条，全文在
`statutes/civil-code-temporal-effect.md`。

判定步骤，逐条对照 `statutes/temporal.yaml`：

1. **取签署日**。`contract-intake` 的四大冻结已冻结该日期。
   签署日未冻结 ⇒ 不得选定任何版本，报「签署日未冻结，无法判定适用法律版本」。
2. **逐部比对**。对每一部打算引用的法，拿签署日与 `applicable_from` /
   `applicable_until` 比对。
3. **落在区间内** ⇒ 可引用本库条文。
4. **落在区间外** ⇒ **禁止引用本库任何条号**，报
   「本库收录的是 X 版（有效期 A~B），该合同签于 C，需人工确认当时文本」。
5. **`applicable_from: unknown_needs_human`** ⇒ 不得假定本库版本适用于任何日期，
   引用前须人工确认施行日。
6. **履行期跨越法律更替日** ⇒ 按时间效力规定第一条第三款（持续性法律事实）
   原则上适用新法，但必须显式披露该判断并触发人工确认。这一步不是比日期能定的。

产物里必须留痕：

```yaml
temporal_gate:
  contract_execution_date: '2019-06-20'      # 来自 intake 冻结
  execution_date_frozen: true
  statutes_version_check:
    - file: arbitration-law.md
      in_library_version: 2025 年修订版
      applicable_from: '2026-03-01'
      contract_date_in_range: false
      decision: citation_forbidden
      statement: 本库收录 2025 修订版（2026-03-01 起），该合同签于 2019-06-20，
                 当时适用 2017 修正版，本库无该版全文 ⇒ 本库未覆盖，需人工确认
  continuing_fact_applied: false
```
...
| 情形 | 正确结论 |
|---|---|
| 签署日 **< 2021-01-01** 的合同引用《合同法》 | **正确，不得报为缺陷**。民法典那天才施行 |
| 签署日 **>= 2021-01-01** 仍引用《合同法》 | 法律引用失效，依据民法典第一千二百六十条 |
| 签署日 **< 2026-03-01** 的合同涉及仲裁协议要件 | 当时是仲裁法**第十六条**，不是本库的第二十七条；本库无那版全文 ⇒ 报未覆盖 |

> **本步骤要防的失败模式只有一个**：拿现行版条号去套旧合同。
> 那样产出的报告看起来一样专业、一样有行号，条号却指向完全不同的内容。
> 比不给结论危险得多。
...
- [ ] **服务范围**：法域为中国大陆；若为港澳台/境外，已终止且未出任何实体结论
- [ ] **法律版本**：报告中每个条号，其所在法律的本库版本区间都覆盖了合同签署日；
      区间外的已改为「本库未覆盖，需人工确认」而**不是**照引现行版条号
- [ ] `temporal_gate` 段已落盘，含签署日、逐部版本校验与 `continuing_fact_applied`
